...
export function* createLead(payload: ReturnType<typeof createLeadRequest>) {
  try {
    put(
      createLeadRequest(
        payload.payload.newLead,
        payload.payload.emailMessage,
        payload.payload.emailMessageTwo
      )
    )
...
export function* sendEmail(payload: ReturnType<typeof sendEmailRequest>) {
  try {
    put(sendEmailRequest(payload.payload.email, payload.payload.title, payload.payload.message))
