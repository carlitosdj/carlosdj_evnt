...
const reducer: Reducer<LeadState> = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case LeadTypes.CREATE_SENDEMAILSINGLE_REQUEST:
      return {...state, loading: true, data: {}}
    case LeadTypes.CREATE_SENDEMAILSINGLE_SUCCESS:
      return {...state, loading: false, error: {}, dataEmailSingle: action.payload.data}
    case LeadTypes.CREATE_SENDEMAILSINGLE_FAILURE:
      return {...state, loading: false, error: action.payload, data: {}}
